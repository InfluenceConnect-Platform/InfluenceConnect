...
export default function OfferPanel({
  ...
  const handleMakeOffer = async () => {
    ...
    setFieldError('');
    try {
      const res = await api.post(`/api/deals/${dealId}/offer`, { amount: parsed });
      setOfferInput('');
      ...
      {isAgreed ? (
        ...
        <div className="px-4 sm:px-5 pb-3 flex flex-col gap-2">
          ...
          {canAccept && latestPending && (
            <div className="flex items-center justify-between gap-3 p-2.5 bg-white border border-amber-200 rounded-xl shadow-sm">
