// Frontend mirror of backend/utils/niches.js — single source of truth for the
// platform's niche taxonomy (9 categories > niches > sub-niches, added 2026-08-08,
// replacing the old flat 28-niche list). Do not duplicate this list elsewhere —
// import from here. See [[niche-taxonomy]] memory for the full picture.

export interface SubNicheDef { slug: string; label: string; }
export interface NicheDef { slug: string; label: string; subNiches: SubNicheDef[]; }
export interface CategoryDef { slug: string; label: string; niches: NicheDef[]; }

export const NICHE_TAXONOMY: CategoryDef[] = [
  {
...
export const NICHE_CATEGORIES: { slug: string; label: string }[] =
  NICHE_TAXONOMY.map(c => ({ slug: c.slug, label: c.label }));

export const NICHES: string[] = NICHE_TAXONOMY.flatMap(c => c.niches.map(n => n.slug));

export const INDUSTRIES: string[] = [...NICHES, 'other'];

export const NICHE_LABELS: Record<string, string> = Object.fromEntries(
  NICHE_TAXONOMY.flatMap(c => c.niches.map(n => [n.slug, n.label]))
);

export const NICHE_CATEGORY: Record<string, string> = Object.fromEntries(
  NICHE_TAXONOMY.flatMap(c => c.niches.map(n => [n.slug, c.slug]))
);

export const SUB_NICHES: string[] = NICHE_TAXONOMY.flatMap(c => c.niches.flatMap(n => n.subNiches.map(s => s.slug)));

export const SUB_NICHE_LABELS: Record<string, string> = Object.fromEntries(
  NICHE_TAXONOMY.flatMap(c => c.niches.flatMap(n => n.subNiches.map(s => [s.slug, s.label])))
);

export const SUB_NICHE_TO_NICHE: Record<string, string> = Object.fromEntries(
  NICHE_TAXONOMY.flatMap(c => c.niches.flatMap(n => n.subNiches.map(s => [s.slug, n.slug])))
);

export const NICHE_SUBNICHES: Record<string, string[]> = Object.fromEntries(
  NICHE_TAXONOMY.flatMap(c => c.niches.map(n => [n.slug, n.subNiches.map(s => s.slug)]))
);

// Tailwind tint classes per niche — cycles through color families already
// covered by the dark-mode bg-x-50 cascade in app/globals.css.
export const NICHE_STYLES: Record<string, string> = {
  "personal-finance-and-budgeting": "bg-blue-50 text-blue-700 border-blue-200",
  "investing-and-wealth-building": "bg-violet-50 text-violet-700 border-violet-200",
  "entrepreneurship-and-startups": "bg-rose-50 text-rose-700 border-rose-200",
  "real-estate-and-property": "bg-pink-50 text-pink-700 border-pink-200",
  "side-hustles-and-freelancing": "bg-orange-50 text-orange-700 border-orange-200",
  "artificial-intelligence-and-automation": "bg-purple-50 text-purple-700 border-purple-200",
  "software-and-coding": "bg-teal-50 text-teal-700 border-teal-200",
  "consumer-electronics-and-hardware": "bg-indigo-50 text-indigo-700 border-indigo-200",
  "digital-productivity": "bg-amber-50 text-amber-700 border-amber-200",
  "gym-training-and-strength": "bg-emerald-50 text-emerald-700 border-emerald-200",
  "endurance-and-specialty-sports": "bg-green-50 text-green-700 border-green-200",
  "holistic-wellness-and-biohacking": "bg-yellow-50 text-yellow-700 border-yellow-200",
  "nutrition-and-specialized-diets": "bg-sky-50 text-sky-700 border-sky-200",
  "mental-health-and-psychology": "bg-fuchsia-50 text-fuchsia-700 border-fuchsia-200",
  "beauty-makeup-and-aesthetics": "bg-lime-50 text-lime-700 border-lime-200",
  "fashion-and-styling": "bg-cyan-50 text-cyan-700 border-cyan-200",
  "daily-vlogging-and-aesthetic-living": "bg-slate-50 text-slate-700 border-slate-200",
  "home-decor-and-interior-styling": "bg-zinc-50 text-zinc-700 border-zinc-200",
  "parenting-and-family-dynamics": "bg-stone-50 text-stone-700 border-stone-200",
  "everyday-and-budget-cooking": "bg-blue-50 text-blue-700 border-blue-200",
  "baking-and-pastry": "bg-violet-50 text-violet-700 border-violet-200",
  "specialty-beverage-and-mixology": "bg-rose-50 text-rose-700 border-rose-200",
  "food-discovery-and-criticism": "bg-pink-50 text-pink-700 border-pink-200",
  "budget-and-adventure-travel": "bg-orange-50 text-orange-700 border-orange-200",
  "mobile-living-and-nomadic-lifestyles": "bg-purple-50 text-purple-700 border-purple-200",
  "specialty-and-luxury-tourism": "bg-teal-50 text-teal-700 border-teal-200",
  "comedy-and-relatable-satire": "bg-indigo-50 text-indigo-700 border-indigo-200",
  "pop-culture-and-media-commentary": "bg-amber-50 text-amber-700 border-amber-200",
  "gaming-ecosystem": "bg-emerald-50 text-emerald-700 border-emerald-200",
  "healthcare-workers": "bg-green-50 text-green-700 border-green-200",
  "educators-and-teachers": "bg-yellow-50 text-yellow-700 border-yellow-200",
  "skilled-trades-and-logistics": "bg-sky-50 text-sky-700 border-sky-200",
  "legal-and-corporate-professionals": "bg-fuchsia-50 text-fuchsia-700 border-fuchsia-200",
  "science-history-and-humanities": "bg-lime-50 text-lime-700 border-lime-200",
  "visual-arts-and-crafting": "bg-cyan-50 text-cyan-700 border-cyan-200",
  "agriculture-plants-and-homesteading": "bg-slate-50 text-slate-700 border-slate-200",
  "pet-and-animal-care": "bg-zinc-50 text-zinc-700 border-zinc-200",
  "automotive-and-mechanical": "bg-stone-50 text-stone-700 border-stone-200",
