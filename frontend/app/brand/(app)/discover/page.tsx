...
/**
 * Free-text search across every city in the taxonomy (not just the active
 * state's list) so a brand can jump straight to e.g. "Kandi" without first
 * knowing it's in West Bengal. Picking a result toggles it via the same
 * `onPick` the checkbox list below uses, and flips the state picker to that
 * city's state so the checkbox list reflects the pick immediately.
 *
 * Portaled + fixed-position like SearchableSelect: this panel lives inside
 * the mobile filter drawer's `overflow-y-auto` body, which would clip a
 * plain `position: absolute` popup once the list runs past the drawer's
 * visible area.
 */
function CitySearchBox({ selectedCities, onPick }: { selectedCities: string[]; onPick: (city: string) => void }) {
  ...
  const pick = (city: string) => {
    onPick(city);
    setQuery('');
    // Keep the panel open so multiple cities can be added in one go — the
    // brand just keeps typing the next name.
    ...
    <div ref={wrapRef} className="relative mb-2.5">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-8 pr-3 py-2 text-sm border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 hover:border-gray-300 transition-all text-gray-900 placeholder:text-gray-400"
        />
      </div>
            ...
            {matches.map(c => {
              ...
              return (
                <div
                  ...
                  onClick={() => pick(c)}
                  className={`flex items-center justify-between gap-2 px-3.5 py-2 text-sm cursor-pointer hover:bg-gray-50 transition-colors ${selected ? 'text-emerald-600 font-semibold' : 'text-gray-700'}`}
                >
  ...
  return (
    <div className="bg-white border border-gray-200/80 rounded-2xl shadow-sm">
      ...
      <div className="p-5">
        ...
        {/* City — search any city directly, or pick a state first and check
            cities within it. Already selected cities stay applied (as
            removable chips) even while browsing a different state's list
            below or searching for another one. */}
        <div className="mb-5">
          <p className="text-xs font-bold text-gray-600 mb-2.5 flex items-center gap-1.5">
          ...
          <p className="text-[11px] text-gray-400 mb-2 -mt-1">or browse by state</p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {isInProgress && (
                          <p className="text-[10px] text-gray-400 dark:text-slate-400 mt-0.5">Deal active — cancel the deal to invite</p>
                        )}
                      </div>
                      {isPending && (
                        <button
      ...
      {showFilters && (
        <div className="lg:hidden fixed inset-0 bg-black/40 backdrop-blur-sm z-40 flex items-end">
          <div className="bg-[#F4F6FB] w-full rounded-t-2xl shadow-2xl max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-blue-50 to-white dark:from-blue-900/30 dark:to-[#0f1e31] rounded-t-2xl border-b border-gray-100 dark:border-slate-700/60 flex-shrink-0">
              <div className="flex items-center gap-2">
            ...
            <div className="relative mb-4">
              ...
              {search && (
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:text-slate-400 dark:hover:text-slate-300 transition-colors cursor-pointer"
                >
